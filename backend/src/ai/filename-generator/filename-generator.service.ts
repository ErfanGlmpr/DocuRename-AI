...
export class FilenameGeneratorService {
  generateSafeFilename(metadata: DocumentMetadata, originalFilename?: string): string {
    let baseName = '';

    if (metadata.suggestedFilename && metadata.suggestedFilename.trim().length > 0) {
      ...
      const refPart = metadata.referenceNumber ? `_${metadata.referenceNumber}` : '';
